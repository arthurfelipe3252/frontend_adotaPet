...
class PetViewModel extends ChangeNotifier {
  ...
  List<Pet> get filteredPets {
    ...
    if (searchQuery.isNotEmpty) {
      result = result.where((p) =>
        ...
        (p.raca?.toLowerCase().contains(searchQuery.toLowerCase()) ?? false),
      ).toList();
    }
    switch (activeFilter) {
      case 'Disponíveis': return result.where((p) => p.status == 'disponivel').toList();
      case 'Em processo': return result.where((p) => p.status == 'em_processo').toList();
      case 'Adotados': return result.where((p) => p.status == 'adotado').toList();
      default: return result;
    }
  }

  void setFilter(String filter) { activeFilter = filter; notifyListeners(); }
  ...
  Future<void> loadPetById(String id) async {
    isLoading = true; error = null; notifyListeners();
    try { selectedPet = await repository.getPetById(id); }
    catch (e) { error = _msg(e); }
    isLoading = false; notifyListeners();
  }
  ...
  Future<bool> createPet(Map<String, dynamic> data) async {
    ...
    try {
      ...
      error = _msg(e); isSaving = false; notifyListeners();
      return false;
    }
  }
  ...
  Future<bool> deletePet(String id) async {
    ...
    try {
      ...
      error = _msg(e); isLoading = false; notifyListeners();
      return false;
    }
  }
}
